...
def segment_signal(df, window_size, step_size):
    ...
    if len(df) < window_size:
        print(f" DEBUG: Not enough data for a full window. Received {len(df)} samples, required {window_size}.")
...
def handle_data():
    ...
    try:
        ...
        for item in data["payload"]:
            if item["name"] == "accelerometer":
                ...
                if "x" in item["values"] and "y" in item["values"] and "z" in item["values"]:
                    ...
                    print(f"⚠ DEBUG: Invalid accelerometer data in payload: {item}")
        ...
        if len(acc_data) >= window_size:
            ...
            if segments.size == 0:
                print("⚠ DEBUG: No valid segments generated.")
